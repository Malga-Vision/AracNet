...
class CustomGCE(nn.Module):
    ...
    def forward(self, outputs: torch.Tensor, targets: torch.Tensor, temperature: Union[None, torch.Tensor]) -> torch.Tensor:  
        outputs = outputs.double() 
        softmax_probs : torch.Tensor = F.softmax(outputs, dim=1)
        if temperature is not None:
            softmax_probs /= temperature
        gathered_preds: torch.Tensor = torch.gather(softmax_probs, dim=1, index=torch.unsqueeze(targets, 1))
        unreduced_loss: torch.Tensor = (1 - gathered_preds ** self.q) / self.q
...
def per_sample_gradient(model: nn.Module, loss_fn: nn.Module, samples: torch.Tensor, targets: torch.Tensor) -> torch.Tensor:
    def compute_sample_loss(params: torch.ParameterDict, buffers: torch.BufferDict, sample: torch.Tensor, target: torch.Tensor):
        batch:      torch.Tensor = sample.unsqueeze(0)
        targets:    torch.Tensor = target.unsqueeze(0)
        predictions = func.functional_call(model, (params, buffers), (batch, ))
        loss = loss_fn(predictions, targets)
        return loss
    
    params:     torch.ParameterDict = {k: v.detach() for k, v in model.named_parameters()}
    buffers:    torch.BufferDict    = {k: v.detach() for k, v in model.named_buffers()}
    ft_compute_grad = func.grad(compute_sample_loss)
    ft_compute_sample_grad = func.vmap(ft_compute_grad, in_dims=(None, None, 0, 0))
    ft_per_sample_grad = ft_compute_sample_grad(params, buffers, samples, targets)

    return ft_per_sample_grad
