...
#os.chdir('./Aracnonet/')
class BFFHQ(Dataset):
    #https://drive.google.com/file/d/1DuZkhQMstWk0nupeYzX9GeIGTAz7M04g/view?usp=sharing" #
    ...
    def get_sampling_weights(self, classes_only: bool):
        if classes_only:
            ...
            group_counts: torch.Tensor = (
                (torch.arange(self.num_groups).unsqueeze(1) == self.group_array)
                .sum(1)
                .float()
            )
    ...
    def set_num_group_and_group_array(self, num_shortcut_category, shortcut_label):
        self.num_group = self.num_classes * num_shortcut_category
    ...
    def get_group_labels(self) -> Generator[None, None, torch.Tensor]:
