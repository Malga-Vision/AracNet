...
if __name__ == "__main__":
    train_set:              Union[CIFAR10C, Waterbirds, BAR, BFFHQ] #type-annot
    ...
    if config["seed"] is not None:
        print(f"Set random_seed={config['seed']}")
    ...
    if dataset=='waterbirds' or dataset=='BAR':
        ...
        train_loader = DataLoader(train_set, batch_size=256, drop_last=False, shuffle=True, pin_memory=True, num_workers=4)
        val_loader   = DataLoader(val_set, batch_size=256, shuffle=False, pin_memory=True, num_workers=4)
    ...
    if train_body_flag ==1 : 
        ...
        try:    
            ...
            base_model_name = train_body(
                ...
                model.device, 
                #0.001
                optimizer=torch.optim.SGD(model.parameters(), lr=0.05, weight_decay=0),
    ...
    #model.load_state_dict(torch.load(os.path.join(save_results_to, f"{base_model_name}_{dataset}_{bias_amount}-biased-final.pth")))
    ...
    base_model_name = learning_from_legs_failure(
        ...
        test_loader,
        model.device, 
        optimizer=lambda params : torch.optim.SGD(params, lr=0.05, momentum=0.9, weight_decay=0, nesterov=True),
        num_classes=num_classes,
        epochs=50,
        wb=wb,
        dataset=dataset
